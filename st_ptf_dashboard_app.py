...
# ========================= PAGE: Closed positions / Realized PnL USD WITH ASSIGNMENT =========================
    ...
    with c1:
        tickers = sorted(df_rlz["ticker"].dropna().unique()) if "ticker" in df_rlz.columns else []
        sel_ticker = st.multiselect("Ticker", tickers)

    with c2:
        asset_opts = sorted(df_rlz["asset_class"].dropna().unique()) if "asset_class" in df_rlz.columns else []
        sel_asset = st.multiselect("Asset class", asset_opts)
    ...
    # --- KPI ---
    def safe_sum(col):
        return pd.to_numeric(df_f.get(col, 0), errors="coerce").fillna(0).sum()

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Realized local", f"{safe_sum('realized_local'):,.2f}")
    k2.metric("Realized USD", f"{safe_sum('realized_usd'):,.2f}")
    k3.metric("Commission local", f"{safe_sum('commission_local'):,.2f}")
    k4.metric("Commission USD", f"{safe_sum('commission_usd'):,.2f}")

    st.caption(f"Rows: {len(df_f)}")

    # --- tabuľka ---
    cols = [
        "instrument", "qty_matched", "open_date", "close_date",
        "open_price", "close_price", "currency_local",
        "realized_local", "realized_usd",
        "commission_local", "commission_usd"
    ]
    cols = [c for c in cols if c in df_f.columns]

    df_disp = df_f[cols].copy().sort_values("close_date", ascending=False)
...
elif page == "📊 Realized PnL Analysis (FIFO, USD)":
